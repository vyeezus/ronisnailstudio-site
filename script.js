...
document.addEventListener('DOMContentLoaded', () => {
    ...
    bookBtn.addEventListener('click', () => {
        ...
        let squareUrl = SQUARE_BASE_URL;
        if (squareServices.length > 0) {
            squareUrl += '?service_ids=' + squareServices.join(',');
        }

        // Open booking.html in a new tab with service names + Square URL
        const bookingUrl = `booking.html?names=${encodeURIComponent(serviceNames.join(','))}&square_url=${encodeURIComponent(squareUrl)}`;
